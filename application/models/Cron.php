...
class Cron extends CI_Model
{
	...
	function get_renewals()
	{
		...
		$cut_off = date('Y-m-d', strtotime('-15 days'));
        ...
        $this->db->select('orders.*, items.*, monthly_payments,	quarterly_payments,	semi_annually_payments,	annually_payments');
		...
		foreach($accounts as $account) {	
			...
			if($account->counter <= $max_payments) {
