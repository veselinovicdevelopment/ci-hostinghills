...
class Order extends CI_Model
{
	...
	{
		self::$db->select('orders.*, items.*, items_saved.server, servers.username AS server_user, servers.type AS server_type, hostname, port, authkey, status.status AS order_status, servers.name AS server_name, reseller_package');
		self::$db->from('orders');
		self::$db->join('items','orders.item = items.item_id','LEFT');
