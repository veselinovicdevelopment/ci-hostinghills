		...
		<input type="hidden" name="o_id" value="<?=$order[0]->o_id?>"> 
				...
				<table class="table table-bordered table-striped">		
						...
						<?php foreach($order as $item) { 
							...
							<input type="hidden" name="item_id[]" value="<?=$item->item_parent?>">
				...
				<h3><?=lang('domains')?></h3>
					<table class="table table-bordered table-striped">		
						...
						<tbody>
							...
							<?php $reg = $this->db->where('item_id', $item->item_parent)->get('items_saved')->row();?>
							<option value=""><?=lang('registrar')?></option>
							<?php if(config_item('resellerclub_live') == 'TRUE') { ?>
							<option value="resellerclub">ResellerClub</option>
							<?php } ?>

							<?php if(config_item('namecheap_live') == 'TRUE') { ?>
							<option value="namecheap">Namecheap</option>
							<?php } ?>

							<?php if(config_item('domainscoza_live') == 'TRUE') { ?>
