								...
								<ul class="list-group alt">
									...
									<div class="col-lg-2">
										<?php if($order->status_id > 5 && $order->registrar != '' && (User::is_admin() || User::perm_allowed(User::get_id(),'manage_accounts'))) {  ?>
													<a href="<?=base_url()."domains/proccess/".$order->registrar?>/register_domain/<?=$order->id?>" class="btn btn-sm btn-success btn-block">
													<?=lang('register')?></a>

													<a href="<?=base_url()."domains/proccess/".$order->registrar?>/transfer_domain/<?=$order->id?>" class="btn btn-sm btn-twitter btn-block">
													<?=lang('transfer')?></a>

													<a href="<?=base_url()."domains/proccess/".$order->registrar?>/renew_domain/<?=$order->id?>" class="btn btn-sm btn-vk btn-block">
													<?=lang('renew')?></a>

											<?php if($order->registrar != 'namecheap') { ?>

													<a href="<?=base_url()?>domains/suspend/<?=$order->id?>" class="btn btn-sm btn-linkedin btn-block" data-toggle="ajaxModal">
													<?=lang('suspend')?></a>

													<a href="<?=base_url()."domains/proccess/".$order->registrar?>/unsuspend_domain/<?=$order->id?>" class="btn btn-sm btn-instagram btn-block">
													<?=lang('unsuspend')?></a>	

											<?php } 
											else { ?>
												<a href="<?=base_url()."domains/proccess/".$order->registrar?>/reactivate_domain/<?=$order->id?>" class="btn btn-sm btn-instagram btn-block">
													<?=lang('reactivate')?></a>	
